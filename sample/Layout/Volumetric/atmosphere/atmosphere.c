...
void analyticEval( const LWVolumeAccess *va, AtmosphereData *dat )
{
   ...
   double tau = 0, fog = 0, x = 0, y = 0, yo = 0, ye = 0, lo = 0, le = 0, h = 0 , nearClip = 0, fogclr[ 3 ], dir[ 3 ];
...
XCALL_( static int )
Atmosphere( int version, GlobalFunc *global, LWVolumetricHandler *local,
   void *serverData )
{
...
static void *ui_get( AtmosphereData *dat, unsigned int vid )
{
   if ( !dat ) return NULL;
...
static int ui_set( AtmosphereData *dat, unsigned int vid, void *value )
{
...
static void button_event( LWXPanelID panel, unsigned int cid )
{
   AtmosphereData *dat;
...
XCALL_( static int )
Atmosphere_UI( int version, GlobalFunc *global, LWInterface *local,
   void *serverData )
{
